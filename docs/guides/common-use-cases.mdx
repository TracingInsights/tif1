...
icon: "list-check"
description: "Solve common F1 data analysis scenarios with tif1"
keywords:
  - "tif1"
  - "Formula 1"
  - "F1 data"
...
for i, (driver, data) in enumerate(quali_results, 1):
    print(f"{i}. {driver}: {data['best_time'].total_seconds():.3f}s ({data['laps']} laps)")
```
...
def compare_telemetry(session, driver1: str, driver2: str):
    ...
    # Get telemetry
    tel1 = d1.get_lap(int(lap1["LapNumber"].iloc[0])).telemetry
    tel2 = d2.get_lap(int(lap2["LapNumber"].iloc[0])).telemetry
...
def analyze_consistency(session, min_laps: int = 10):
    ...
    for driver in clean["Driver"].unique():
        ...
        # Calculate metrics (seconds keep mean, std, and range plain floats)
