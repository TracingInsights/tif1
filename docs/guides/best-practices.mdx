...
icon: "star"
description: "Apply recommended tif1 patterns for performance, reliability, and code quality"
keywords:
  - "best practices"
...
Follow these practices for the best performance, reliability, and code quality.
...
graph TD
    ...
    AsyncLoad -->|Yes| ParallelFetch[Parallel Fetch<br/>3x+ faster]
...
Control what data gets fetched to optimize performance. Load only the necessary data.

<CodeGroup>
```python Good - Selective Loading
import tif1

# Load only laps data, skip telemetry/weather/messages
session = tif1.get_session(2021, "Belgian Grand Prix", "Race")
#session.load(laps=True, telemetry=False, weather=False, messages=False) - loading the data is not necessary, you can directly use `session.laps`
laps = session.laps  # Fast - only laps fetched
```

```python Avoid - Loading Everything
session = tif1.get_session(2021, "Belgian Grand Prix", "Race")
session.load()  # Fetches everything by default
laps = session.laps  # Slower - unnecessary data fetched
```
</CodeGroup>
...
# Parallel fetch - validated at 3x+ vs sequential
session = tif1.get_session(2021, "Belgian Grand Prix", "Race")
tels = session.get_fastest_laps_tels(by_driver=True)

# tels is a DataFrame with all drivers' fastest lap telemetry
for driver in tels["Driver"].unique():
    driver_tel = tels[tels["Driver"] == driver]
    analyze_telemetry(driver_tel)
```
...
for driver in session.drivers:
    ...
    tel = driver_obj.get_lap(int(fastest["LapNumber"].iloc[0])).telemetry
...
# index=False skips the row labels; the Laps reset_index override does not
# support the in-place reset_index that pandas to_sql uses for the index
with sqlite3.connect("f1_data.db") as conn:
    # SQLite column names are case-insensitive, so drop the duplicate
    # "Team"/"team" and "Driver"/"drv" source columns before writing
    laps_sql = laps.loc[:, ~laps.columns.str.lower().duplicated()]
    laps_sql.to_sql("laps", conn, if_exists="replace", index=False)
```
...
Before processing multiple sessions, check the data that is available.
...
def batch_process():
    ...
    # Continue processing
    for event in events:
        session = tif1.get_session(2021, event, "Race")
        process(session)
```
...
# Or use the FastF1-compatible API
from tif1.fastf1_compat import Cache
Cache.clear_cache()  # Clears active cache
```
...
Do not hit the real CDN in unit tests.
...
def test_lap_filtering():
    ...
    session = tif1.get_session(2021, "Belgian Grand Prix", "Practice 1")
    laps = session.laps
...
Add docstrings and comments to the analysis code.
