...
print(f"VER: {ver_fastest['LapTime'].iloc[0].total_seconds():.3f}s")
print(f"LEC: {lec_fastest['LapTime'].iloc[0].total_seconds():.3f}s")
```
...
lap = ver.get_lap(int(lap_number))
...
When loading data for the first time, use async for parallel fetching (more than 3x faster than sequential in the benchmark suite):
...
# Fetch all telemetry in parallel (validated at 3x+ vs sequential)
...
laps_pl = session_pl.laps  # Returns polars DataFrame
```
...
plt.plot(ver_laps["LapNumber"], ver_laps["LapTime"].dt.total_seconds(), marker="o")
...
# Replace <cache-dir> with the platform-specific tif1 cache directory
ls -la <cache-dir>/

# Fix permissions
chmod -R 755 <cache-dir>/
```
