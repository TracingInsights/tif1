...
<Info>
  ...
  When enabled, requests `.min.json` files which are 20-40% smaller.
</Info>

### TIF1_CDN_BASE_URL

Legacy single CDN configuration (deprecated in favor of TIF1_CDNS).
...
<Warning>
  ...
  Consider using `TIF1_CDNS` for multi-CDN support with automatic fallback.
</Warning>
