...
<Info>
  This page documents the DataFrame columns you work with in tif1. They are built from raw JSON files stored in the TracingInsights data repos, which use short abbreviated keys (e.g. `s1`, `vi1`, `wWS`). Every raw field is documented field-by-field in the [Data Reference](/reference/data-reference).
</Info>
