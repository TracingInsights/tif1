...
description: "Answers to frequently asked questions about tif1"
keywords:
  - "FAQ"
...
tif1 is a high-performance Python library for Formula 1 data analysis. It provides fast access to timing, telemetry, and weather data from 2018-current. Parallel async loading is more than 3x faster than sequential fetching in the benchmark suite.
...
- **Speed**: Parallel data loading with async/HTTP/2 (3x+ vs sequential, benchmark-validated)
...
Yes, tif1 is open source under the MIT License. Use it freely for personal, academic, or commercial projects.
...
No, tif1 does not require any API keys or authentication. All data is publicly available via the TracingInsights CDN.
...
The first access to a session downloads data from the CDN. Subsequent loads are instant from the cache. Use `laps_async()` for parallel initial loads (3x+ vs sequential in the benchmark suite).
...
- The data has not been published yet (check whether the session ended recently)
...
- Firewall blocking the CDN hosts (StaticDelivr, jsDelivr, Hugging Face)
...
### Column Names Do Not Match fastf1

tif1 uses PascalCase for all columns, for example `LapTime` instead of `time`. This is intentional for consistency. Use the exact column names shown in the data schema reference.
...
No, tif1 uses lazy loading. Data is fetched automatically at first access:
...
For explicit control over the data that is fetched, use `session.load()`.
...
```python
# Parallel loading - faster than sequential (3x+ in benchmarks)
...
Sector analysis shows where time is gained or lost:
...
tif1 works with Streamlit for interactive dashboards:
...
tif1 provides plotting utilities, and custom visualizations are also possible:
...
1. Check whether the bug is already reported in [GitHub Issues](https://github.com/TracingInsights/tif1/issues)
...
MIT License. Use it freely in any project.
