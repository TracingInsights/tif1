...
icon: "database"
description: "Describes the tif1 multi-layer cache: memory cache, SQLite cache, and CDN fetch, plus configuration, warming, and maintenance"
keywords:
  - "caching strategy"
  - "cache architecture"
  - "SQLite cache"
...
3. **Sharing**: Process-local memory caches cannot be shared, while disk caches enable multi-process coordination.
4. **Graceful Degradation**: If one cache layer fails, the system falls back to the next layer automatically.
5. **Optimal Resource Usage**: Hot data stays in fast memory. Warm data stays on disk. `tif1` fetches cold data on demand.
...
A typical analysis session accesses 5-10 race sessions with multiple data types. Caching reduces the total load time for these sessions from **30-60 seconds to under 1 second**.

## Cache Architecture

The `tif1` caching system consists of two primary layers that work together, with an optional third layer for distributed deployments. Each layer serves a specific purpose in the performance hierarchy.
...
When `tif1` receives a data request (for example, `session.laps`), the system follows this flow:
...
The first and fastest cache layer is an in-memory LRU (Least Recently Used) cache. It extends the Python `functools.lru_cache` decorator with custom enhancements. This cache stores recently accessed DataFrames and metadata objects directly in process memory.
...
<AccordionGroup>
  <Accordion title="Size Configuration">
    Set the memory cache size to match the workload:
...
The second cache layer is a SQLite database that provides persistent, disk-based storage for cached data. This layer is positioned between the fast but volatile memory cache and the slow but reliable network fetches.

### Architecture & Implementation

The SQLite cache is a single-file database with an optimized schema and indexes:
...
The default cache location follows the FastF1 platform layout, using `tif1` as the application directory:
...
<AccordionGroup>
  <Accordion title="Cache Location">
    Choose the cache location for the deployment:

    - **Local development**: Use the tif1 OS-dependent default cache directory
    - **Shared server**: Use a shared directory (for example, `/shared/cache/tif1/`)
    - **Docker**: Mount a volume for persistence
...
Correct use of the cache system is necessary for optimal performance. This section covers common operations, workflows, and patterns.

### Reading from Cache

The cache system operates transparently. Applications do not check or manage cache hits and cache misses explicitly. The system handles the cache hierarchy automatically:
...
When the data structure changes (for example, new columns), the cache system invalidates old entries automatically:
...
The cache system can check the CDN for data updates with HTTP headers:
...
The cache system detects and removes corrupted entries automatically:
...
Cache warming pre-populates the cache with data before the data is needed. Warming eliminates cold-start latency and ensures optimal performance from the first request.
...
3. **Better User Experience**: Dashboards and applications load without cold-start delay
...
Warm the cache based on usage patterns:
...
Cache performance analysis guides optimization. This section provides detailed performance metrics, benchmarks, and analysis techniques.
...
Profile cache performance in the application:
...
<AccordionGroup>
  ...
  <Accordion title="Application-Level Optimization">
    Optimize cache usage in the application:
...
For read-only deployments (for example, serverless and immutable infrastructure):
...
If cache errors or corrupted data occur:
...
If the cache does not work as expected:
...
If SQLite connection problems occur:
...
<AccordionGroup>
  ...
  <Accordion title="'Failed to decompress'">
    **Cause**: Corrupted compressed data in the cache
  ...
  <Accordion title="'Permission denied'">
    **Cause**: Insufficient permissions on the cache directory
...
<CardGroup cols={2}>
  ...
  <Card title="Use SSD Storage" icon="hard-drive">
    SQLite cache performs 10-100x better on SSD vs HDD. Place the cache on fast storage.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Reuse Session Objects" icon="recycle">
    Avoid redundant fetches by reusing session objects. Memory cache access takes under 1ms.
  </Card>
...
<CardGroup cols={2}>
  <Card title="Use Shared Cache" icon="share-nodes">
    Configure a shared cache directory for multi-process applications to maximize the hit rate.
  </Card>

  <Card title="Persistent Storage" icon="database">
    Mount the cache on persistent volumes in Docker or Kubernetes to survive restarts.
  </Card>

  <Card title="Backup Regularly" icon="floppy-disk">
    Back up the cache database periodically. Test the restoration process.
  </Card>
...
<Steps>
  <Step title="Enable Memory Cache">
    Ensure the memory cache is enabled with an appropriate size (100-500 items).
  </Step>

  <Step title="Use SSD Storage">
    Place the SQLite cache on an SSD for 10-100x better performance.
  </Step>
...
<AccordionGroup>
  <Accordion title="File Permissions">
    Set appropriate permissions on the cache directory:
  ...
  <Accordion title="Encryption at Rest">
    For sensitive deployments, encrypt the cache:
...
The `tif1` caching system is a production-grade solution. It maximizes performance and maintains data integrity and reliability. Correct configuration of the multi-layer cache architecture provides:
...
- **Get Help**: Join the [Discord](https://discord.gg/tif1) or the [GitHub Discussions](https://github.com/tracinginsights/tif1/discussions)
...
<CardGroup cols={3}>
  ...
  <Card title="Configuration" href="/reference/environment-variables" icon="gear">
    Configure the cache for a specific use case.
  </Card>
...
<Note>
**Performance Tip**: For the best performance, enable memory cache, use SSD storage, and warm frequently accessed data. This can provide 1000-3000x speedup compared to fetching from the CDN.
</Note>

<Warning>
**Important**: Always monitor cache size and enable automatic cleanup to prevent disk space issues. Set `cache_max_size_mb` to an appropriate limit for the deployment environment.
</Warning>

<Info>
**Season storage**: The `tif1` cache system stores an entire F1 season in ~720 MB of compressed storage. The season covers 24 races, all sessions, and all data types. The uncompressed data occupies over 12 GB. Cached access to this data is immediate.
</Info>
