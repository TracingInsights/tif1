...
graph TD
    ...
    J --> K[jsDelivr CDN Primary]
    K -->|Success| L[JSON Data]
    K -->|Failure| M[GitHub Fallback]
    M --> L
    L --> N[orjson Parser]
...
**CDN Manager (`cdn.py`)**
- Constructs URLs for TracingInsights data repositories
- Primary: jsDelivr CDN (`cdn.jsdelivr.net/gh/TracingInsights/{year}@main`)
- Fallback: Direct GitHub API (rate limited to 60 req/hour for unauthenticated)
...
**Fallback Source: GitHub API**
- URL Pattern: `https://api.github.com/repos/TracingInsights/{year}/contents/{path}`
...
# Result: https://cdn.jsdelivr.net/gh/TracingInsights/2025@main/laps/monaco/race/driver_VER.json
...
async def fetch_with_fallback(url_path):
    ...
    try:
        github_url = f"https://api.github.com/repos/TracingInsights/{year}/contents/{url_path}"
        response = await http_client.get(github_url, timeout=30)
        if response.status_code == 200:
            # GitHub API returns base64-encoded content
            import base64
            content_b64 = response.json()["content"]
            content = base64.b64decode(content_b64)
            logger.info(f"GitHub API success: {url_path}")
            return content
    ...
    raise tif1.NetworkError(
        f"Failed to fetch {url_path} from all sources",
        url=url_path,
        attempts=2
    )
```
