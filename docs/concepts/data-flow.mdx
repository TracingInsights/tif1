...
description: "How data moves through tif1, from CDN requests to DataFrame delivery. Covers multi-tier caching, HTTP/2 fetching, orjson parsing, and type optimization."
keywords:
  - "data flow"
  - "architecture"
  - "data pipeline"
...
This document explains how data moves through tif1, from the first CDN request to the final DataFrame delivery. It covers caching mechanisms, network protocols, data transformations, and performance optimizations. These details help with performance tuning, troubleshooting, and data loading decisions.

tif1 treats performance as its core principle. The multi-tier caching system, HTTP/2 multiplexing, and async parallel fetching all minimize latency and maximize throughput. Each section explains what happens, why it happens, and how to use these systems.
...
graph TD
    ...
    L -->|Failure| N{Try Hugging Face}
    N -->|Success| M
    N -->|Failure| V{Try StaticDelivr}
    V -->|Success| M
    V -->|Failure| W[Fail]
    M --> P[Schema Validator]
    P --> Q[orjson Parser]
    Q --> R[DataFrame Constructor]
    R --> S[Column Transformer]
    S --> T[Data Enrichment]
    T --> U[Type Optimization]
    U --> X[Save to SQLite]
    X --> Y[Save to Memory]
    Y --> D
...
- A 404 from one CDN falls through to the next (mirrors can be stale); `DataNotFoundError` is raised when every CDN refuses the request with a 4xx client error (404 included), so everywhere-missing files do not burn the retry/backoff budget. 5xx and transport errors keep the retryable `NetworkError`
...
compressed = zstandard.ZstdCompressor(level=1).compress(json_bytes)
# Typical compression ratio: ~4:1 (better than zlib-3, ~4.5x faster)

# Deserialization (read from cache)
json_bytes = zstandard.ZstdDecompressor().decompress(compressed)
...
async def fetch_with_fallback(url_path):
    ...
    # Attempt 1: jsDelivr CDN (primary)
    try:
    ...
    # Attempt 3: Hugging Face bucket (backup)
    try:
        hf_url = f"https://huggingface.co/buckets/tracinginsights/{year}/resolve/{url_path}"
        response = await http_client.get(hf_url, timeout=30)
        if response.status_code == 200:
            logger.info(f"Hugging Face success: {url_path}")
            return response.content
        logger.warning(f"Hugging Face returned {response.status_code}")
    except Exception as e:
...
tif1 uses the `niquests` library. `niquests` supports HTTP/2 and multiplexes requests over a single TCP connection.
...
The tif1 data flow architecture is designed to evolve. Planned enhancements include:
...
The architecture is transparent, observable, and tunable. For a simple analysis script or a production data pipeline, tif1 provides the needed performance and reliability.

For questions, issues, or contributions, visit the [GitHub repository](https://github.com/TracingInsights/tif1) or join the [community discussions](https://github.com/TracingInsights/tif1/discussions).
