...
async def load_session_data():
    ...
    # Faster than synchronous loading (3x+ in benchmarks)
...
# Switch the default backend globally (persists via config.save();
# pass lib="polars" to get_session for a one-off switch instead)
...
ver_avg = ver_laps["LapTime"].mean().total_seconds()
ham_avg = ham_laps["LapTime"].mean().total_seconds()
...
for i, (_, lap) in enumerate(best_laps.iterrows(), 1):
    print(f"{i}. {lap['Driver']}: {lap['LapTime'].total_seconds():.3f}s ({lap['Compound']})")
```
