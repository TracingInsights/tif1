...
The default cache follows FastF1's platform layout, using `tif1` as the application directory: Windows `%LOCALAPPDATA%/Temp/tif1`, macOS `~/Library/Caches/tif1`, and Linux/other POSIX `~/.cache/tif1` when `~/.cache` exists (otherwise `~/.tif1`). Set `TIF1_CACHE_DIR` or `cache_dir` in `.tif1rc` to override it.
