...
<CardGroup cols={2}>
  <Card title="Blazing Fast" icon="bolt">
    Parallel async loading: more than 3x faster than sequential fetching (benchmark-validated), with HTTP/2 multiplexing.
  </Card>
...
3. **Batteries included.** 22 optional one-call `plot_*()` chart helpers turn raw data into ready-made charts and analyses — `tif1.plot_top_speeds(...)`, `tif1.plot_driver_laptimes(...)`, and more.
