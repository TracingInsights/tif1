...
graph TB
    ...
    HTTP --> SRC[CDN Sources: StaticDelivr / jsDelivr / HuggingFace]
    SRC --> Data[JSON Data]
...
- `tif1.payload_loader`: single payload-loading pipeline (memo → cache → CDN fetch → validate → write-back) behind an injectable `HttpTransport` seam (`NiquestsTransport` in production, `InMemoryTransport` in tests).
...
sequenceDiagram
    ...
    alt Cache Hit
        ...
        Session->>CDN: Fetch JSON from CDN sources (StaticDelivr, jsDelivr, Hugging Face)
...
1. **CDN Fetch**: Parallel HTTP/2 requests to the CDN source chain (StaticDelivr primary, jsDelivr and Hugging Face fallbacks)
