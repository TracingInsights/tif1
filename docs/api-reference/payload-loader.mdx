...
Raises `DataNotFoundError` when the payload does not exist, `InvalidDataError` when the payload is not a JSON object or failed validation, and `NetworkError` when all CDN sources failed. Validation failures always raise.
