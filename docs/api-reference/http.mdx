...
- **URL Formatting**: Automatic URL construction for TracingInsights data repositories with format `{cdn_base}/{year}@main/{gp}/{session}/{path}`
...
2. **Network Fetch**: Tries primary CDN (jsDelivr), falls back to secondary CDNs on failure
