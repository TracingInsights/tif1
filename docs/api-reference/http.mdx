...
icon: "globe"
description: "Technical reference for HTTP sessions, async fetching, connection pooling, retries, circuit breakers, and CDN fallback in tif1"
keywords:
  - "tif1"
  - "Formula 1"
  - "F1 data"
...
The HTTP and networking layer is the foundation of `tif1`'s data fetching infrastructure. It is engineered for performance, reliability, and resilience. It loads Formula 1 telemetry and session data from remote CDN sources. The networking stack is the result of years of optimization work for F1 data loading. Cold starts can require fetching 100+ telemetry files in parallel while maintaining sub-second response times.

The networking layer is optimized for the patterns of F1 data access. These patterns include high-concurrency parallel fetching, connection reuse, retry strategies, and graceful degradation under failure conditions. A full session (20 drivers × 50+ laps) yields 1000+ telemetry files. The system loads such a session in 2-5 seconds on a typical broadband connection.

### Architecture Philosophy

The networking layer is built on four core principles that guide every design decision:

1. **Performance First**: Every design decision prioritizes speed and throughput. Connection pooling eliminates TCP handshake overhead (saving 30-50ms per request). HTTP/2 multiplexing allows multiple requests over a single connection. Keep-alive optimization maintains persistent connections for up to 1000 requests. Zero-copy JSON parsing with `orjson` provides 2-5x faster parsing than the standard library. These optimizations compound to deliver sub-second response times even when fetching 100+ files in parallel.

2. **Resilience by Design**: Network failures are expected, not exceptional. The circuit breaker pattern prevents cascading failures by blocking requests during sustained outages. Exponential backoff with jitter reduces load on failing services while allowing recovery. Multi-CDN fallback ensures graceful degradation when primary sources fail. The system can tolerate complete CDN failures. It recovers automatically when services come back online.

3. **Resource Efficiency**: Thread pools with dynamic sizing minimize resource consumption. Connection reuse reaches 90%+ reuse rates. Caching also lowers resource use. The system can handle 128+ concurrent requests during telemetry prefetch without overwhelming the connection pool. File descriptors are not exhausted.
...
The networking stack provides a complete set of features:
...
- **Dynamic Connection Pooling**: Pool size automatically scales based on configured concurrency. The minimum is 256 connections, up to 512+ for burst handling
...
The retry mechanism uses exponential backoff, jitter, and multi-CDN fallback:
...
Reliable CDN management with automatic failover, health tracking, and optional minification:

- **Multiple CDN Support**: Configure multiple CDN sources with priority ordering via `cdns` config array
- **Automatic Failover**: Switches to backup CDNs on failure without user intervention
...
- **Dynamic Enable/Disable**: CDNs can be dynamically disabled based on error patterns (for example, HTTP/2 multiplexing incompatibility after redirects)
...
The default CDN configuration uses jsDelivr as the primary source. Hugging Face buckets are the fallback, and StaticDelivr is a last-resort backup:
...
Configure multiple CDNs for redundancy:
...
This can reduce file sizes by 20-40%, which improves load times on slow connections. Minification adds a small processing overhead on the CDN side. It is disabled by default. Fast connections perform better without this overhead.
...
Always use a proper CDN like jsDelivr that is designed for high-volume content delivery.

#### 6. Rate Limiting and Concurrency Control

Prevents CDN throttling and manages resource consumption:

- **Configurable Concurrency**: Default 22 concurrent requests, up to 128+ for telemetry prefetch
...
- **Process Pool Parsing**: Optional multi-process JSON parsing for large payloads. It is disabled by default for telemetry because of IPC overhead
...
The networking layer is configurable via environment variables or `.tif1rc` file. All networking-related settings can be tuned for each specific use case, from ultra-fast cold starts to resource-constrained environments.
...
- `max_retries` (int, default: 3): Maximum retries per CDN. Set to 0 for zero-retry mode (ultra-fast cold start).
- `max_workers` (int, default: 20): Thread pool size for async operations. The executor is sized as the max of `max_workers` and the concurrency caps (`max_concurrent_requests`, telemetry prefetch).
...
- `keepalive_max_requests` (int, default: 10000): Maximum requests per connection before recycling.

**Concurrency Control:**
- `max_concurrent_requests` (int, default: 22): Maximum concurrent HTTP requests for general operations.
...
- Maximum 10000 requests per connection (configurable via `keepalive_max_requests`)
...
- **Privacy**: DNS queries are encrypted and cannot be intercepted or logged by ISPs
...
Customize the DNS resolver order in the `.tif1rc` file:
...
HTTP/2 multiplexing improves F1 data loading in several ways:
- **Single Connection**: All requests to a CDN use one TCP connection instead of 20+
- **No Head-of-Line Blocking**: Slow requests do not block fast ones
...
Some CDNs have issues with HTTP/2 multiplexing after redirects. An error like `"non-multiplexed response after a redirect"` disables the CDN automatically. Multiplexing can also be disabled manually:
...
HTTP/3 provides benefits for high-latency or lossy networks, but may have compatibility issues with some CDNs or corporate firewalls. To solve connection issues, try disabling HTTP/3:
...
<Note>
  The HTTP session is automatically managed and thread-safe. Direct interaction is rarely needed. The async fetch layer handles all session management internally.
</Note>

<Warning>
  Do not close the session manually while other code still uses it. The session is shared across the entire application and automatically cleaned up on program exit.
</Warning>
...
This function performs a graceful shutdown of the HTTP session, closing all active connections and releasing resources. It is automatically registered with `atexit` for cleanup on program exit, so manual calls are rarely necessary.
...
**When to Use:**
- Testing scenarios that need a reset of session state
...
<Note>
  Session cleanup is automatically registered with `atexit`, so manual cleanup is usually unnecessary. The session closes properly when the program exits.
</Note>
...
This function provides real-time metrics about connection pool usage. The metrics show how effectively connections are reused. High reuse rates (>80%) indicate efficient operation, while low rates may suggest configuration issues.
...
<Note>
  Connection statistics are automatically logged every 60 seconds (configurable via `connection_stats_log_interval`). Use this function for on-demand monitoring or custom logging.
</Note>
...
This function clears all connection metrics, resetting counters to zero. It is primarily useful in testing scenarios. Use it to measure statistics for a specific operation without interference from previous requests.
...
The async fetch module provides high-performance parallel data loading. It uses Python's `asyncio` with thread pool executors and optional process pool JSON parsing. This layer is optimized for F1 data loading patterns. Cold starts may require fetching 100+ telemetry files at once.
...
This is the core async fetching function that powers all data loading in `tif1`. It implements a multi-stage pipeline optimized for performance and reliability.

**Pipeline Stages:**

1. **Cache Lookup**: Checks in-memory cache first (&lt;1ms), then SQLite cache (~5-10ms)
2. **Network Fetch**: Tries primary CDN (jsDelivr), falls back to Hugging Face buckets, then StaticDelivr on failure
...
**Zero-Retry Mode**: When `max_retries=0`, the function tries each CDN once without delays, achieving 50-70% faster cold starts. This mode is ideal for ultra-fast startup when occasional transient failures are acceptable.

**Pool Exhaustion Handling**: When the connection pool is exhausted, automatic backoff is applied (0.5-5s with jitter) before retrying. This prevents cascading failures.

**Parameters:**

- `year` (int): Season year (for example, 2024)
- `gp` (str): Grand Prix name (for example, "Bahrain Grand Prix")
- `session` (str): Session name (for example, "Race", "Qualifying", "FP1", "FP2", "FP3", "Sprint")
- `path` (str): Path to JSON file relative to session directory
  ...
  - `max_retries` (int | None): Maximum retries per CDN
...
- `DataNotFoundError`: If data does not exist (404 from all CDNs)
  - Includes year, event, and session for context
  - Not retried (404 is a fatal error indicating missing data)
  - Common for sessions that have not been run yet or invalid driver codes
...
- **Zero-Retry Mode**: 50-70% faster cold starts - tries each CDN once with no delays
...
<Note>
  This function is the foundation of all data loading in `tif1`. The high-level `Session.load()` API uses this internally for all network operations. Direct calls are rarely needed, except when building custom data loading logic.
</Note>

<Warning>
  Setting `max_retries=0` enables zero-retry mode for ultra-fast cold starts, but increases the chance of transient failures. Use this mode only when speed is critical and occasional failures are handled gracefully.
</Warning>
...
This function is the core of parallel data loading, enabling efficient fetching of 100+ files simultaneously. It is optimized for telemetry prefetch scenarios. Use it to load lap data and telemetry for all drivers in a session.
...
- `validate_payload` (bool): If True, run payload validation before returning data
  ...
  - `max_retries` (int | None): Maximum retries per request
  - Default: Uses `max_retries` config value (typically 3)
...
- `max_concurrent_requests` (int | None): Maximum concurrent requests
  - Default: Uses `max_concurrent_requests` config value (typically 22)
...
async def load_multiple_drivers():
    ...
    # Fetch all in parallel (default 22 concurrent)
...
<Note>
  This function is used internally by `Session.load()` for parallel driver lap data loading and telemetry prefetch. It is the key to `tif1`'s fast cold start performance, enabling 2-5 second session loads even with 100+ telemetry files.
</Note>

<Warning>
  High concurrency (128+) can overwhelm slow networks or trigger CDN rate limiting. Start with default settings (22) and increase gradually while monitoring connection stats and success rates.
</Warning>
- `validate_payload`: If True, run payload validation before returning data
- `max_retries`: Maximum retries per request
- `timeout`: Request timeout in seconds
- `max_concurrent_requests`: Maximum concurrent requests (defaults to config value, typically 22)
...
- Maximum 22 concurrent requests by default (configurable via `max_concurrent_requests` config)
...
2. **Tune max_concurrent_requests to match the network**: Higher values (50-128) for fast connections, lower (5-10) for slow.
...
The connection pool is central to `tif1`'s networking performance. Tune it to match the workload. Correct tuning has a direct effect on performance.

**How Connection Pooling Works:**

For each HTTP request, the session checks if an existing connection to that host is available in the pool:
...
With HTTP/2 multiplexing enabled, multiple requests can share a single connection. This improves pool efficiency.

**Pool Sizing Guidelines:**

The pool size should match the concurrency requirements:
...
For very large payloads (>1MB), enable multi-process JSON parsing:
...
HTTP/2 multiplexing is one of the most impactful optimizations in `tif1`. Learn how it works to gain the full benefit.
...
3. **Better Throughput**: No head-of-line blocking (slow requests do not block fast ones)
...
Some CDNs have issues with HTTP/2 multiplexing after redirects. Such errors look like this:

```python
MultiplexingError: non-multiplexed response after a redirect
```

The CDN is disabled automatically. Multiplexing can also be disabled manually:
...
The circuit breaker prevents cascading failures by blocking requests during sustained outages. Learn its state machine to use it effectively.
...
Monitoring of these metrics helps identify bottlenecks and optimize performance.
...
This section provides solutions to common networking issues in `tif1`.
