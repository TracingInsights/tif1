...
- **Configurable Concurrency**: Default 64 concurrent requests, up to 128+ for telemetry prefetch
...
**Concurrency Control:**
- `max_concurrent_requests` (int, default: 64): Maximum concurrent HTTP requests for general operations.
...
- `max_concurrent_requests` (int | None): Maximum concurrent requests
  - Default: Uses `max_concurrent_requests` config value (typically 64)
...
async def load_multiple_drivers():
    ...
    # Fetch all in parallel (default 64 concurrent)
...
<Warning>
  High concurrency (128+) can overwhelm slow networks or trigger CDN rate limiting. Start with default settings (64) and increase gradually while monitoring connection stats and success rates.
</Warning>
- `validate_payload`: If True, run payload validation before returning data
- `max_retries`: Maximum retries per request
- `timeout`: Request timeout in seconds
- `max_concurrent_requests`: Maximum concurrent requests (defaults to config value, typically 64)
...
- Maximum 64 concurrent requests by default (configurable via `max_concurrent_requests` config)
