...
- **StaticDelivr Primary**: Primary CDN with jsDelivr as fallback for improved reliability
- **URL Formatting**: Automatic URL construction for TracingInsights data repositories (StaticDelivr: `{cdn_base}/{year}/main/{gp}/{session}/{path}`, jsDelivr: `{cdn_base}/{year}@main/{gp}/{session}/{path}`)
...
The default CDN configuration uses StaticDelivr as the primary source with jsDelivr as fallback:
...
2. **Network Fetch**: Tries primary CDN (StaticDelivr), falls back to secondary CDN (jsDelivr) on failure
