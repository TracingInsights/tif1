...
icon: "network-wired"
description: "Advanced HTTP session management with intelligent connection pooling, DNS-over-HTTPS fallback, and comprehensive performance monitoring"
keywords:
  - "tif1"
  - "Formula 1"
  - "F1 data"
...
**Thread Safety:**
The function uses a module-level lock (`_session_lock`) to ensure thread-safe initialization. Once the session is created, subsequent calls return the cached instance without acquiring the lock, making this operation extremely fast in the common case.
