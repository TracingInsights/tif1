...
- Hugging Face bucket sources never use minification. These sources do not serve `.min.json`. A 404 there would waste a round trip (the fallback chain would have to hit another source) on every request, so Hugging Face sources are always excluded from minification
...
5. For each source (in priority order):
   ...
   d. If DataNotFoundError (404):
      - Fall through to the next source (mirrors can be stale or divergent)
      - Do NOT increment the failure count (not a CDN health problem)
...
6. If all sources fail:
   - Raise DataNotFoundError if every failure was a 404 (data does not exist)
   - Otherwise raise NetworkError with details
   ↓
...
3. **Try each source** (in priority order):
   ...
   - **On `DataNotFoundError` (404)**: Fall through to the next source (mirrors can be stale or divergent, so the payload may exist elsewhere); the failure count is NOT incremented
