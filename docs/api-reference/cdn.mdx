...
title: "CDN API"
icon: "cloud"
description: "Multi-source CDN management with automatic fallback, health tracking, and source selection"
keywords:
  - "tif1"
  - "Formula 1"
  - "F1 data"
...
Optional JSON minification reduces file sizes by 20-40% by removing whitespace and formatting. This improves download speeds and reduces bandwidth costs, especially for large telemetry datasets.
...
- Hugging Face bucket sources never use minification. These sources do not serve `.min.json`. A 404 there would abort the fallback chain with a fatal `DataNotFoundError`
...
12. **High-availability requirements**: Add multiple backup sources for production applications

<Warning>
  **Never use `raw.githubusercontent.com` as a CDN source.** GitHub's raw content URLs have strict rate limits (60 requests/hour for unauthenticated requests). tif1 explicitly blocks these URLs during initialization. Use jsDelivr or another proper CDN service instead.
...
<Info>
  The default configuration uses three sources in priority order. The sources are **StaticDelivr** (primary), **jsDelivr** (fallback), and **Hugging Face buckets** (`https://huggingface.co/buckets/tracinginsights`, last resort):
...
Returns the global CDN manager singleton instance. This function always returns the same `CDNManager` object, ensuring consistent CDN state across the entire application.
...
The CDN manager is thread-safe for read operations (getting sources, checking health). Modify sources or mark failures from a single thread, or use appropriate synchronization:
...
The CDN manager initializes automatically on the first call to `get_cdn_manager()`. During initialization, the following steps occur:
...
Internal dictionary tracking consecutive failure counts for each CDN source. When a source's failure count reaches `_max_failures` (default: 3), it is automatically excluded from `get_sources()` until reset.
...
Returns a list of now available CDN sources, sorted by priority (lowest priority number first). This method filters out sources that should not be used for data fetching.
...
Marks a CDN source as having failed a request. This increments the failure counter for the source. When a source reaches the failure threshold (default: 3 consecutive failures), it is automatically excluded from `get_sources()` until reset.
...
This method enables the self-healing behavior of the CDN system. When an unhealthy source serves a request, its failure count resets. The source is then used normally again.
...
Resets all failure counts for all CDN sources to 0. This re-enables all previously disabled sources. Use this method to recover from widespread network issues or to give all sources a fresh start.
...
3. **Try each source** (in priority order):
   ...
   - **On `DataNotFoundError` (404)**: Re-raise immediately (data does not exist, no point trying other sources)
...
**Raises:**
- `DataNotFoundError`: If resource does not exist (404 from any source)
  - This is re-raised immediately without trying other sources
  - Indicates the requested data genuinely does not exist
...
1. **Data not found (404)**: Permanent error—data does not exist
   - Re-raised immediately as `DataNotFoundError`
   - No point trying other CDN sources (they will all return 404)
