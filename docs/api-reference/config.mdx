...
icon: "gear"
description: "Configure tif1 with config files, environment variables, and the runtime API"
keywords:
  - "configuration"
  - "tif1 config"
...
The `tif1` configuration system controls every part of the library's behavior. It covers network settings, caching strategies, data validation, and performance settings. The configuration architecture is flexible. It allows tuning for a specific use case: low-latency data access, high-throughput batch processing, or development debugging.

This guide covers all configuration options, their interactions, performance implications, and best practices for different deployment scenarios. Interactive Jupyter notebooks, production data pipelines, and CI/CD environments have different requirements. This guide helps with the configuration of `tif1` for each one.

### Configuration Philosophy

`tif1` is built for **performance-first** operation. The default configuration values give good out-of-the-box performance for most use cases. The library also exposes dozens of tuning parameters for advanced use. These parameters minimize latency or maximize throughput for a specific workload.

The configuration system follows a **layered override model**. Settings can be specified at multiple levels with clear precedence rules:

1. **Default values** — Hardcoded defaults optimized for general use
2. **Configuration file** (`.tif1rc`) — Persistent settings stored in the user's home directory
...
Each layer overrides the previous one. This model supports different environments (development, staging, production). It also supports different use cases (interactive analysis, batch processing, CI/CD pipelines).

### Configuration Architecture

The configuration system is implemented as a **singleton pattern**. This pattern ensures that all parts of an application share the same configuration state. This design provides several benefits:
...
Consider customizing the configuration when:

- **Performance is critical** — The workload needs minimal latency or maximum throughput for time-sensitive applications
- **Network conditions vary** — A slow connection, a corporate proxy, DNS issues, or specific network requirements apply
- **Resource constraints exist** — Memory usage, connection pools, or concurrent requests need limits due to system limitations
- **Data validation is needed** — Data issues need debugging, quality checks need implementing, or compliance needs strict validation
- **Caching behavior matters** — Fresh data, shared storage, or access patterns need different cache settings
- **Backend preferences differ** — Polars gives better performance than pandas, or a workflow needs specific DataFrame behavior
...
- **Workload characteristics change** — The workload switches between interactive analysis (low concurrency) and batch processing (high concurrency)
- **Compliance requirements exist** — Data storage locations need control, certain features need disabling, or regulatory requirements apply
- **Debugging is required** — Verbose logging, validation, or specific diagnostic information is needed
...
- Reliable retry and circuit breaker settings
...
This layered approach works as follows. Set baseline configuration in a file. Override specific settings per environment with environment variables. Make dynamic adjustments at runtime based on workload characteristics.
...
**Security Note:** By default, the library does **not** load `.tif1rc` from the current working directory. This prevents malicious config files in untrusted directories. Set `TIF1_TRUST_CWD_CONFIG=true` only if the current directory is trusted.
...
The configuration system performs validation when values are **read** (via `get()`), not when they are **set**. This design allows any value to be set programmatically. Invalid values are rejected when accessed, falling back to the provided default. Validation includes:
...
Specify a custom configuration file location using the `TIF1_CONFIG_FILE` environment variable:
...
For project-specific configuration, place a `.tif1rc` file in the project directory. For security reasons, this file is **only loaded** if it is explicitly enabled:
...
<Warning>
  Only set `TIF1_TRUST_CWD_CONFIG=true` in trusted directories. Malicious `.tif1rc` files could modify library behavior in unexpected ways.
</Warning>
...
- Comments are not supported (JSON does not allow comments)
- Trailing commas are not allowed

**Invalid File Handling:**

If the configuration file is invalid (malformed JSON, wrong type), the library logs a warning. It then continues with default values. The library never crashes due to invalid configuration files.
...
The file can also be created manually using any text editor:
...
For different environments or use cases, maintain multiple configuration files. Switch between them using environment variables.
...
Returns the global singleton configuration instance. The `Config` object is a singleton. There is only one instance per Python process. All calls to `get_config()` return the same object. This ensures configuration consistency across the entire application.

**Returns:**
- `Config` — The global configuration singleton instance

**Thread Safety:**
The configuration object is thread-safe for reading. However, modifying configuration values (`config.set()`) during concurrent operations may lead to race conditions. It is recommended to configure the library once at startup before spawning threads or processes.
...
The `Config` object provides three primary methods for configuration values: `get()` for reading, `set()` for modifying, and `save()` for persisting changes to disk.

### `get(key, default=None)`

Retrieve the value for a specific configuration key. This method includes built-in validation for many configuration keys. It ensures values are within acceptable ranges and of the correct type.
...
- `default` (Any, optional) — Default value to return if the key does not exist or validation fails. Defaults to `None`.

**Returns:**
- `Any` — The configuration value, or `default` if the key does not exist or validation fails
...
Update a configuration value in memory for the current Python session. Changes made with `set()` are **not persisted** to disk unless `save()` is explicitly called.
...
- Changes are **session-only** unless `save()` is called
- No validation is performed during `set()` — validation happens in `get()`
- Custom keys that are not part of the default configuration can be set
...
<Note>
  Changes made with `set()` only affect the current Python session. To make changes permanent, call `save()` after setting the desired values.
</Note>
...
Persist the current in-memory configuration to a JSON file on disk. This makes configuration changes permanent across Python sessions.
...
<Note>
  The `save()` method writes the **entire** configuration, not just the modified keys. This ensures the saved file is a complete, self-contained configuration.
</Note>
...
<ResponseField name="user_agent" type="str" default="tif1/{version}">
  User-Agent header sent with all HTTP requests. Useful for identifying the application in server logs or implementing custom rate limiting.

  **Default:** `tif1/{version}` (for example, `tif1/0.2.0`)

  **Use Cases:**
  - Identify the application in CDN logs
...
<ResponseField name="json_parse_workers" type="int" default="0">
  ...
  <Warning>
    Process-pool JSON parsing can hurt performance due to IPC overhead for telemetry-heavy workloads. Keep it disabled unless a specific use case requires it.
  </Warning>
...
While not part of the `Config` object, the log level can be set using `setup_logging`:
...
This section provides configuration recipes for common use cases, with detailed explanations of why each setting is chosen.
...
- Reliable retry settings handle transient failures
...
**Symptoms:**
- Changes made with `config.set()` do not seem to work
...
2. Verify network bandwidth is not saturated
...
4. Check disk space
5. Verify SQLite timeout is not too short
...
1. **Start with defaults** — The default configuration is optimized for most use cases. Only change settings for a specific need.
...
5. **Document the configuration** — Keep a README or comments explaining why specific settings were chosen.
...
3. **Balance concurrency and resources** — Higher concurrency is not always better. Find the right level for the system and network.
...
5. **Enable caching** — Cache improves performance for repeated queries. Only disable for debugging or when fresh data is critical.
...
2. **Configure reliable retry settings** — Use exponential backoff with jitter to handle transient failures gracefully.
...
1. **Trust config files carefully** — Only set `TIF1_TRUST_CWD_CONFIG=true` in trusted directories.

2. **Use DoH for privacy** — DNS-over-HTTPS encrypts DNS queries, improving privacy on untrusted networks.

3. **Validate config files** — Ensure config files are valid JSON and do not contain malicious values.

4. **Limit cache locations** — Store cache in secure locations with appropriate permissions.

5. **Custom user agents** — Use descriptive user agents to identify the application in logs.
...
The `tif1` configuration system controls library behavior through multiple configuration sources with clear precedence rules. Key takeaways:
...
- **Production** — Balanced settings, reliable retries, monitoring
...
The configuration system is flexible and easy to use. Interactive notebooks, production pipelines, and specific workloads all have different needs. The configuration system provides the tools to tune `tif1` for each use case.
...
<CardGroup cols={2}>
  <Card title="Caching Strategy" href="/concepts/caching-strategy">
    Learn about the multi-layer caching system and its configuration options.
  </Card>
  ...
  <Card title="HTTP Session" href="/api-reference/http">
    HTTP session configuration and connection pooling in detail.
  </Card>
  <Card title="Installation" href="/installation">
    Setup guide and initial configuration.
  </Card>
  <Card title="CLI Configuration" href="/concepts/cli">
    Configure tif1 via the command-line interface.
  </Card>
...
Use this script to validate the configuration:
...
A: No, the default configuration works well for most use cases. Only configure for specific performance, reliability, or resource requirements.

**Q: What is the difference between config file and environment variables?**
...
A: Delete or rename the `.tif1rc` file and unset all `TIF1_*` environment variables. The library then uses hardcoded defaults.

**Q: Why is not my configuration taking effect?**
...
A: Polars is faster for large datasets but uses more memory. Pandas has lower memory overhead and better compatibility. Start with pandas, then switch to polars when more performance is needed.
...
**Q: What is ultra cold start mode?**

A: Ultra cold start skips loading full session data when only specific data is needed. It reduces initial load time by 50-80%. Enable for interactive analysis, disable for batch processing.
...
A: Yes, use `config.set()` to change values at runtime. Changes take effect immediately but are not persisted unless `config.save()` is called.

**Q: What is the recommended production configuration?**

A: See the "Production Service Configuration" in the "Configuration Patterns and Recipes" section for a complete production configuration.
...
**Q: What is the difference between `pool_connections` and `pool_maxsize`?**
...
A: Enable for common access patterns (for example, accessing driver laps after loading session). Disable for memory-constrained environments or when access patterns are unpredictable.

**Q: What is the recommended cache size?**
...
**Q: What is the impact of HTTP/2 multiplexing?**

A: HTTP/2 multiplexing reduces connection overhead by reusing connections for multiple requests. Enabled by default. Disable it only if specific proxies or CDNs cause issues.
...
**Q: What is the circuit breaker for?**
