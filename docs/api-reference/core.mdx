...
<Info>
  The core module is the center of tif1. It provides the primary interface for loading and working with Formula 1 data. All data access flows through the `Session` object. The `Session` object gives access to laps, telemetry, weather, race control messages, and driver information. The API is designed for performance with async support, caching, and lazy loading patterns.
</Info>
...
- **Multi-Layer Caching**: Caching strategy with in-memory LRU cache (instant access) and SQLite persistent cache (survives restarts). Cache keys are session-specific and backend-aware (pandas vs polars).

- **Async Support**: All data-intensive operations have async variants (`laps_async()`, `get_fastest_laps_tels_async()`) that use parallel HTTP requests. The benchmark suite validates more than 3x faster loading compared to sequential fetching.

- **Backend Flexibility**: Choose pandas (default, full compatibility) or polars (faster for large datasets) for DataFrame operations. Polars gives a 10-100x speedup for some operations. Backend selection is per-session and affects all data structures.
...
- **Ultra-Cold Mode**: Optimization mode that bypasses cache validation and skips loading unnecessary data for minimal latency. Use it in serverless environments or when only specific data is needed (for example, fastest lap telemetry).

- **Error Resilience**: Complete exception hierarchy (`DataNotFoundError`, `NetworkError`, `InvalidDataError`) with structured error context. Failed telemetry fetches are tracked per-driver to avoid repeated failures.
...
| `laps_async()` | 0.5-1s | &lt;100ms | Explicit async, parallel loading |
...
<ParamField path="enable_cache" type="bool | None" default="None">
  ...
  **Cache location**: tif1's OS-dependent cache directory (configurable via `TIF1_CACHE_DIR`)
</ParamField>
...
<ParamField path="lib" type="Literal['pandas', 'polars'] | None" default="None">
  ...
  **Performance note**: Polars can be faster for large datasets (10-100x for some operations), but requires the `polars` package. If polars is requested but not available, tif1 falls back to pandas with a warning.
</ParamField>

**Returns:**

<ResponseField name="Session" type="Session">
  A configured `Session` object ready to load data. The session is not loaded until its properties are accessed (lazy loading) or `session.load()` is called explicitly.
</ResponseField>
...
<ResponseField name="ValueError" type="Exception">
  ...
  - Session name/number does not exist for the specified event
  - Session number is out of range for the event
</ResponseField>

<ResponseField name="DataNotFoundError" type="Exception">
  Raised when the specified year, GP, or session does not exist in the data source.
</ResponseField>
...
<ParamField path="gp" type="str" required>
  URL-encoded Grand Prix name (for example, `"Monaco_Grand_Prix"`). Spaces should be replaced with underscores or `%20`.

  **Note**: `get_session()` handles this encoding automatically.
</ParamField>

<ParamField path="session" type="str" required>
  URL-encoded session name (for example, `"Qualifying"`, `"Race"`).
</ParamField>
...
<ResponseField name="year" type="int">
  The season year (for example, 2025).
</ResponseField>

<ResponseField name="gp" type="str">
  URL-encoded Grand Prix name (for example, "Monaco_Grand_Prix").
</ResponseField>
...
<ResponseField name="drivers" type="list[str]">
  List of driver numbers as strings in the session (for example, ["1", "33", "44"]).
  Loaded lazily on first access.

  <Note>
    This returns driver NUMBERS, not driver codes. For driver codes (for example, "VER", "HAM"), use `drivers_df["Driver"]`.
  </Note>
</ResponseField>
...
<ResponseField name="drivers_df" type="DataFrame">
  ...
  - `Abbreviation`: 3-letter driver code (for example, "VER", "HAM")
  - `TeamName`: Team name
...
<ResponseField name="race_control_messages" type="DataFrame">
  ...
  - `Category`: Message category (for example, "Flag", "SafetyCar")
...
<ResponseField name="name" type="str">
  Human-readable session name (for example, "Monaco Grand Prix - Qualifying").
</ResponseField>
...
<ResponseField name="total_laps" type="int | None">
  ...
  <Note>
    This property is not yet implemented and now returns `None`.
  </Note>
</ResponseField>
...
**Parameters:**
- `driver`: 3-letter driver code (for example, "VER") or driver number (for example, 33)

**Returns:**
- `Driver` object

**Raises:**
- `DriverNotFoundError`: If the driver does not exist in this session
...
Parallel fetch telemetry for multiple drivers' fastest laps. This is faster than fetching telemetry sequentially.
...
<Tip>
  **Performance Tip:** This method fetches telemetry for multiple drivers in parallel. This is faster than calling `get_fastest_lap_tel()` sequentially for each driver.
</Tip>
...
Async version of `fetch_all_laps_telemetry()`. Fetches telemetry for all laps in parallel, faster than the sync version.
...
<Warning>
  **Memory Warning:** Fetching all telemetry for a full race session can consume 50-200 MB of memory. Use this only when all telemetry data is needed. For most use cases, fetch telemetry for specific laps or drivers instead.
</Warning>
...
<ResponseField name="driver" type="str">
  The 3-letter driver code (for example, "VER", "HAM", "LEC").
</ResponseField>
...
- `driver["DriverNumber"]`: Car number as string (for example, "33", "44", "16")
- `driver["Abbreviation"]`: 3-letter driver code (same as `driver.driver`)
- `driver["TeamName"]`: Team name (for example, "Red Bull Racing", "Mercedes")
...
print(f"Fastest: {fastest['LapTime'].iloc[0].total_seconds():.3f}s")
...
- `[column_name]`: Direct column access (for example, `result["Driver"]`)
...
<Tip>
  **Performance Tip:** `laps_async()` is faster than the `laps` property for cold starts
...
<Note>
  **Ultra-cold mode** bypasses cache validation and skips loading unnecessary data for faster cold starts.
  Use when minimal latency is needed and not all session data is required. The trade-off is that some
...
<Tip>
  **Best Practices:**
  ...
  - Use `ultra_cold=True` when only specific data is needed
