...
icon: "code"
description: "Type definitions, schemas, and type hints in tif1"
keywords:
  - "tif1"
  - "Formula 1"
  - "F1 data"
...
`tif1` is built with type safety as a core design goal. Every public API, function, method, and data structure includes type hints. These hints enable static type checkers (mypy, pyright, pytype) to catch errors before runtime. This design provides multiple benefits:
...
This guide covers all type definitions in `tif1`, organized into the following sections:
...
Literal types are a key Python type system feature. They restrict values to a specific, finite set of allowed strings. This provides compile-time validation and autocomplete support in IDEs. Unlike plain `str` types, Literal types provide:
...
The `SessionType` literal defines all valid Formula 1 session types across different weekend formats. Formula 1 has evolved its weekend structure over the years, introducing sprint races and varying qualifying formats. This type ensures that only sessions present in the F1 calendar can be referenced.
...
Formula 1 weekend formats have changed over time, especially with the introduction of sprint races:
...
TypedDict is a Python feature that provides type hints for dictionary structures. Unlike regular dictionaries, TypedDict allows the exact structure to be specified:
- Exact keys that must or may exist
- Type of value for each key
- Which fields are required vs optional

`tif1` uses TypedDict to define the structure of DataFrame row data. This provides:
...
The `LapDataDict` TypedDict defines the complete structure of lap timing data in DataFrames. This is the largest data structure in `tif1`, containing timing, speed, tire, weather, and metadata for each lap.
...
Complete example using all field categories:
...
Use mypy or pyright in the development workflow:
...
This guide covered all type definitions in `tif1`:
...
6. **Best Practices** include always using type hints, using Union types, importing from tif1.types, and enabling strict type checking
...
1. **Enable Type Checking**: Add mypy or pyright to the project
2. **Add Type Hints**: Annotate tif1 code with proper types
3. **Use IDE Features**: Use autocomplete and inline documentation
