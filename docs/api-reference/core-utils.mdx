...
<Note>
  The `core_utils` package provides the foundational utilities that power `tif1`'s high-performance data processing pipeline. While these are primarily internal APIs, they expose capabilities for advanced users. Advanced users gain fine-grained control over data transformations, backend conversions, and performance optimization.

  This module is the performance engine of `tif1`. It implements zero-copy operations, optimized JSON parsing, and DataFrame transformations. These features let the library process millions of telemetry rows with minimal overhead.
</Note>

## Overview

The core utilities package (`tif1.core_utils`) is organized into five specialized modules. Each module addresses a critical aspect of the library's performance and functionality:
...
- **Type safety** with full validation and error handling
- **Performance-first** design with optimized hot paths
- **Backend agnostic** APIs that support both pandas and polars
...
- **SIMD-friendly operations**: Use NumPy/Arrow vectorization
- **Lazy computation**: Defer work until absolutely necessary
- **Zero-copy semantics**: Share memory between backends via Arrow

**Real-world impact**: These optimizations enable `tif1` to load and process a full race weekend with telemetry for all 20 drivers. A full weekend covers 3 practice sessions, qualifying, and the race. The load completes in under 5 seconds on modern hardware.

---

## Backend Conversion

The backend conversion module provides zero-copy DataFrame transformations between pandas and polars using Apache Arrow as the interchange format. This enables switching between backends without the performance penalty of traditional serialization/deserialization.
...
- `rechunk` (`bool`, optional): Whether to rechunk the resulting polars DataFrame for optimal memory layout. Default is `False` to preserve zero-copy semantics. Set to `True` to perform many operations on the polars DataFrame with contiguous memory.
...
<Tip>
  Use `rechunk=False` (default) for quick conversions with only a few operations. Use `rechunk=True` for workloads with many polars operations.
</Tip>
...
<Warning>
  When `use_pyarrow=True`, the resulting pandas DataFrame uses PyArrow extension arrays. Some legacy pandas operations may not support these arrays. If compatibility issues occur, set `use_pyarrow=False` to use native pandas dtypes.
</Warning>
...
Convert a DataFrame to the target backend (pandas or polars) using zero-copy conversion when possible. This is the high-level API that automatically detects the source backend and performs the appropriate conversion.
...
The JSON utilities module provides high-performance JSON parsing and serialization. It uses orjson, a fast, correct JSON library for Python written in Rust. This module is critical for `tif1`'s performance because it parses large JSON payloads from the CDN. These payloads contain lap data, telemetry, and race control messages.
...
**Raises:**
- `TypeError`: If the data contains non-serializable types (for example, custom classes without `__dict__`)
...
4. **Benchmark each use case**: Profile the specific JSON payloads to measure impact
...
The helpers module (`helpers.py`) provides utility functions for DataFrame manipulation, validation, and data transformation. These functions carry most of `tif1`'s data processing work. They handle input validation and complex DataFrame operations across both pandas and polars backends.

### Design Philosophy

The helper functions follow these core principles:

1. **Backend Agnostic**: All functions work with both pandas and polars DataFrames
2. **Zero-Copy Optimization**: Minimize memory allocations and avoid unnecessary data copies
3. **Type Safety**: Full validation with clear error messages
...
Input validation is critical for data integrity and user experience. The validation helpers provide complete checks with informative error messages.
...
<Note>
  The data loading pipeline performs this validation early, before any network requests are made. Early validation fails fast and provides clear feedback.
</Note>
...
<Tip>
  Use `None` instead of an empty list to indicate "all drivers". An empty list is considered an error because it would result in an empty DataFrame, which is likely unintentional.
</Tip>
...
<Warning>
  Lap numbers in F1 start from 1, not 0. When iterating over laps, use `range(1, num_laps + 1)` instead of `range(num_laps)`.
</Warning>
...
- `param_name` (`str`): Human-readable parameter name for error messages. Used to provide context in error messages (for example, "gp", "session_type", "driver").
...
3. Cannot be whitespace-only (for example, `"   "`, `"\t"`, `"\n"`)
...
URL-encode a string component for safe use in CDN URLs. This function properly encodes special characters, spaces, and international characters according to RFC 3986. The encoded URLs stay valid and do not break when passed to HTTP clients.

**Parameters:**
- `component` (`str`): String to encode. Can contain any Unicode characters, spaces, or special characters.

**Returns:**
- `str`: URL-encoded string with all special characters percent-encoded (for example, space becomes `%20`, `&` becomes `%26`).
...
These functions provide backend-agnostic operations for DataFrame manipulation with both pandas and polars.
...
<Note>
  This function prefers concrete type checking over the `lib` parameter. Some code paths can surface pandas DataFrames even when the configured backend is polars (for example, during backend conversion).
</Note>
...
<Tip>
  Use `_create_empty_df()` instead of `pd.DataFrame()` or `pl.DataFrame()` directly for backend-agnostic code. This ensures consistency and handles the polars availability check automatically.
</Tip>
...
Filter laps DataFrame to include only rows with valid lap times, and add a numeric `LapTimeSeconds` column for analysis. This function is critical for data quality. It removes invalid laps (pit laps, out laps, deleted laps) and provides a consistent numeric representation of lap times.
...
Rename DataFrame columns based on a mapping dictionary, with automatic handling of duplicates and backend-specific APIs. This function is used to transform CDN column names to user-facing FastF1-compatible names.
...
- **No-op renames**: Skips renames where source == target (for example, `{"Driver": "Driver"}`)
...
<Tip>
  The function automatically handles edge cases like duplicate column names and missing columns. This makes it safe to use with varying CDN data formats across different years and sessions.
</Tip>
...
The library defines complete column rename mappings to transform CDN data format to user-facing FastF1-compatible format. These mappings handle both verbose and abbreviated column names from different CDN data versions.
...
List of columns that should be converted to categorical dtype for memory optimization. These columns have low cardinality (few unique values) and benefit from categorical encoding.
...
The resource manager module (`resource_manager.py`) provides a pattern for managing resources with guaranteed cleanup. The cleanup runs even when initialization fails partway through. This is critical because it prevents resource leaks in error scenarios.
...
- **Complete logging**: All cleanup operations are logged for debugging
...
Cleanup all registered resources in reverse order (LIFO). This method tries to clean up all resources even if individual cleanup operations fail. Cleanup errors are logged but do not prevent other resources from being cleaned up.
...
6. Clear the resource list after all cleanup work finishes
...
The ResourceManager provides detailed logging for debugging:
...
<Note>
  ResourceManager is used internally by the library for managing HTTP sessions, cache connections, and other resources. Most users do not need to interact with it directly. It is available for advanced use cases requiring custom resource management.
</Note>
