...
title: "Models"
icon: "box"
description: "Laps, Lap, Driver, Telemetry, SessionResults, DriverResult, and CircuitInfo classes"
keywords:
  - "data models"
  - "Laps model"
  - "Telemetry model"
...
The models module provides an object-oriented interface for working with Formula 1 data. These classes extend pandas DataFrame and Series objects with domain-specific methods for F1 analysis. Filter, analyze, and visualize racing data with few lines of code.

## Overview

All model classes in tif1 are built on top of pandas data structures. They provide familiar DataFrame and Series interfaces plus specialized methods for F1 data manipulation. This design allows pandas operations alongside tif1-specific functionality.
...
1. **Simple Access Patterns**: Natural property access (for example, `lap.telemetry`, `driver.laps`)
...
4. **Session Context**: All models maintain a reference to their parent session for direct data access
5. **Performance First**: Optimized for speed with vectorized operations and caching
...
**Module:** `tif1.models` (canonical module; re-exported by `tif1.core` for backward compatibility)

**Thread Safety:** Read operations are thread-safe; write operations should be synchronized externally

### Common Access Patterns

The `Laps` class can be accessed through multiple pathways, depending on the analysis needs:
...
Each row in a `Laps` DataFrame represents a single lap with complete timing and metadata. The DataFrame contains the following column categories:

#### Core Identity Columns
- **`Driver`** (str): 3-letter driver code (for example, "VER", "HAM", "LEC")
- **`DriverNumber`** (str): Car racing number (for example, "1", "44", "16")
- **`Team`** (str): Full team name (for example, "Red Bull Racing", "Mercedes")
...
The Laps class provides a complete set of filtering methods that can be chained together for complex queries. All filtering methods return new Laps objects, preserving the original data (immutable operations). These methods are optimized for performance using pandas' vectorized operations.
...
- **`identifier`**: Can be one of several formats for maximum flexibility:
  - **`str`**: 3-letter driver code (for example, "VER", "HAM", "LEC") - most common usage
  - **`int`**: Driver racing number (for example, 1, 44, 16)
...
**Behavior Details:**
- Accepts mixed identifier types in the same list (for example, `["VER", 44, {"driver": "LEC"}]`)
...
- **`only_by_time`** (bool, default=False): Reserved for future use. Now has no effect. In FastF1, this parameter controls whether to consider only lap time or also track status. In tif1, only lap time is considered.
...
- **`laps`**: Can be:
  ...
  - **`slice`**: Range of laps (for example, `slice(10, 20)` for laps 10-20 inclusive)
...
- Returns empty Laps if lap number does not exist
...
- **`name`** (str): Full team name (for example, "Red Bull Racing", "Mercedes", "Ferrari")
...
- Returns empty Laps if columns do not exist
...
**Returns:**
- **`pick_not_deleted()`**: Laps that were not deleted by race control
...
- Returns all laps if the respective column does not exist
...
Access high-frequency telemetry data for the laps in the collection. This feature enables detailed analysis of driver inputs and car behavior.
...
Iterate over laps with optional required columns. This is the recommended way to process each lap individually.
...
- Raises `KeyError` if a required column does not exist in the DataFrame
...
Split qualifying laps into Q1, Q2, and Q3 sessions. Note: tif1 does not split by session internally, so this returns copies for FastF1 compatibility.
...
- If the `QualifyingSession` column exists, splits by that column
...
The `Laps` class inherits all pandas DataFrame operations, so standard pandas methods work:
...
**Important:** When pandas operations return DataFrames, the result may be a plain pandas DataFrame rather than a Laps object. Use the tif1-specific methods (pick_*) to maintain the Laps type and session context.
...
The `Lap` class represents a single lap with timing data and access to high-frequency telemetry. It inherits from `pandas.Series`. The result is a row-like object with named fields, accessed like a dictionary or via attribute access.
...
A `Lap` object is a pandas Series containing all the lap data columns described in the Laps section. Access any column value with dictionary-style or attribute-style access:
...
<ResponseField name="telemetry" type="Telemetry">
  ...
  **Behavior:**
  - Loaded lazily on first access (not fetched until this property is accessed)
...
Get weather data for this lap. Now returns an empty Series as lap-specific weather data is included in the lap's columns.
...
The `Driver` class represents a driver's participation in a session. It provides access to the driver's metadata, laps, and analysis methods. It inherits from `pandas.Series`, making it a dictionary-like object with driver information.
...
- **`DriverNumber`** (str): Racing number (for example, "1", "44", "16")
- **`Abbreviation`** (str): 3-letter driver code (for example, "VER", "HAM", "LEC")
- **`TeamName`** (str): Full team name (for example, "Red Bull Racing", "Mercedes")
- **`TeamColor`** (str): Team color hex code (for example, "#3671C6")
- **`FirstName`** (str): Driver's first name (for example, "Max", "Lewis")
- **`LastName`** (str): Driver's last name (for example, "Verstappen", "Hamilton")
- **`FullName`** (str): Driver's full name (for example, "Max Verstappen")
...
<ResponseField name="driver" type="str">
  The 3-letter driver code (for example, "VER", "HAM", "LEC"). This is stored as an attribute (not in the Series data) and provides quick access to the driver identifier.
...
**Raises:**
- **`LapNotFoundError`**: If the lap does not exist for this driver
...
The `Telemetry` class represents high-frequency data recorded throughout a lap or multiple laps. It inherits from `pandas.DataFrame` with additional slicing and analysis methods specifically designed for telemetry data. This class enables detailed analysis of driver inputs, car behavior, and racing lines.
...
<ResponseField name="driver" type="str">
  Driver code for this telemetry data (for example, "VER", "HAM"). May be None if telemetry is from multiple drivers.
</ResponseField>
...
- Returns empty Telemetry if time window does not exist
...
Slice telemetry using a boolean mask. Useful for custom filtering based on any condition.
...
**Raises:**
- **`ValueError`**: If mask length does not match telemetry length
...
**Behavior Details:**
- Now returns placeholder data (not fully implemented in tif1)
- Provided for FastF1 compatibility
...
**Parameters:**
- **`rule`** (str, default="1S"): Resampling frequency (for example, "1S" = 1 second, "100ms" = 100 milliseconds)
- **`**kwargs`**: Additional arguments (for compatibility)
...
### Example 1: Complete Race Analysis
...
Lazy-loading dictionary that fetches telemetry data per driver on demand. Constructed with the owning session (any `TelemetryProvider`); indexing by driver code fetches and caches that driver's telemetry on first access.

```python
telemetry_by_driver = session.telemetry  # LazyTelemetryDict
ver_tel = telemetry_by_driver["VER"]     # fetches on first access
```
...
The narrow session interface consumed by the model family (`laps`, `weather_data`, `get_fastest_laps_tels`). `tif1.core.Session` satisfies the protocol structurally as the production adapter; tests can provide lightweight in-memory implementations instead. Decorated with `typing.runtime_checkable` so structural conformance can be asserted with `isinstance`.
...
All models support full type hints for better IDE integration, autocomplete, and type checking. This makes development faster and reduces errors.
...
The models module provides a complete, simple API for F1 data analysis:

### Key Takeaways

1. **Laps**: Primary interface for lap timing data with flexible filtering methods
