...
title: "API Overview"
icon: "book-open"
description: "A map of the tif1 programmatic interface"
keywords:
  - "tif1 API"
  - "F1 API reference"
  - "Session API"
...
The `tif1` API is a modern Python library for Formula 1 data analysis, telemetry processing, and motorsport analytics. Performance is the primary focus of the design. `tif1` maintains full compatibility with the `fastf1` library. It often loads data 5-10x faster. The speed comes from async operations, multi-layer caching, parallel HTTP fetching, and an optional Polars backend.

`tif1` serves data scientists, motorsport engineers, application developers, and racing enthusiasts. These users can work efficiently with Formula 1 data at scale.
...
print(f"Sampling rate: {len(telemetry) / driver.get_fastest_lap()['LapTime'].iloc[0].total_seconds():.1f} Hz")
...
4. **CDN URL Construction**
   ...
   - Uses a three-source CDN chain (StaticDelivr primary, jsDelivr fallback, Hugging Face buckets backup) pointing to TracingInsights GitHub data repositories
...
The primary entry point for loading Formula 1 session data. The function accepts multiple input formats. It also provides fine-grained control over what data gets loaded.
...
Control exactly what data gets loaded to optimize performance for each specific use case:
...
Retrieve the complete event schedule for a championship year. The schedule includes all Grand Prix events with metadata, dates, locations, and session structures.
...
The `Session` object represents an entire Formula 1 session (Practice, Qualifying, Sprint, or Race). It is the primary container for all session data.
...
print(f"Average lap time: {laps['LapTime'].mean().total_seconds():.3f}s")

# Get fastest lap
fastest = driver.get_fastest_lap()
print(f"Fastest lap: {fastest['LapTime'].iloc[0].total_seconds():.3f}s")
...
- **[Best Practices](/guides/best-practices)** - Optimize analysis code
