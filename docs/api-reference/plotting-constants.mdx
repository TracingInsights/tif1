...
The `plotting_constants` module holds the plotting color data: historically accurate team colors and tyre compound colors for each F1 season from 2018 onwards. The `tif1.plotting` module reads these mappings when it resolves colors and car image codes. The module contains data only; it defines no functions.
...
`TEAM_CODES` maps a season year to a mapping of timing-data team names to the short car image codes used by the bundled car artwork in `tif1.assets`. Names with engine suffixes map to the same code, so every team name variant that appears in lap data resolves to the correct artwork.
...
- `team_color_mapping(year)` returns a copy of `TEAM_COLORS[year]`, expanded so that every team name with a car code in `TEAM_CODES` also has a color.
- `get_team_code(identifier, session=None, *, year=None, exact_match=False)` resolves a team name to its car image code, using the session year when a session is given, otherwise the explicit `year`.
