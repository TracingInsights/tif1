...
icon: "check-circle"
description: "Pydantic-based validation system for F1 data integrity and quality assurance"
keywords:
  - "tif1"
  - "Formula 1"
  - "F1 data"
...
The validation module provides a Pydantic-based validation system. It ensures data integrity and catches malformed responses from the CDN before they reach the application code.

## Overview

The tif1 validation system acts as a quality gate between raw CDN JSON data and the application. It performs deep structural validation, type checking, and anomaly detection. This ensures that the data is clean and consistent.
...
- **Out-of-Range Values**: Validates numeric constraints (for example, RPM &lt; 20,000, gear &lt;= 8, stint &gt;= 1)
...
Validates lap timing data structure with full field checking and length consistency validation.
...
**Purpose:**
Validates raw lap timing JSON from the CDN. Ensures that all required fields are present and that arrays have consistent lengths. Values must meet domain constraints (for example, stint >= 1, tire life >= 0).
...
Validates high-frequency telemetry data in batch mode (faster than point-by-point validation).
...
**Purpose:**
Validates driver roster data from the CDN. Ensures that all driver codes follow the 3-letter format and that required metadata fields are present.
...
**Validation Rules:**
- `driver`: Must be exactly 3 uppercase letters (for example, "VER", "HAM", "LEC")
...
Pydantic model for lap timing data with automatic length consistency validation.
...
Pydantic model for high-frequency telemetry data with automatic unwrapping of nested structures.
...
**Anomaly Types:**
- **MISSING_LAPS**: Gaps in lap number sequence (for example, laps 1, 2, 4, 5 - missing lap 3)
- **DUPLICATE_LAPS**: Same lap number appears multiple times
- **OUTLIER_TIMES**: Lap times far from the average (>3x mean)
...
**Parameters:**
- `laps` (list[dict]): List of lap dictionaries with at least one of the `lap` or `time` fields
...
- **low**: Minor issues that do not affect analysis (for example, statistical outliers from pit stops)
- **medium**: Moderate issues that may affect completeness (for example, missing laps)
- **high**: Critical issues that indicate data corruption (for example, duplicate lap numbers)
...
- Validation errors are logged but do not raise exceptions
...
2. **Handle validation errors gracefully**: Do not crash on bad data.
...
5. **Use null-like string conversion**: The validation module automatically converts "", "none", "null", "nan" to None.
