...
`tif1` provides comprehensive Jupyter notebook integration with rich HTML displays, automatic environment detection, and interactive utilities designed specifically for Formula 1 data analysis. The library automatically enhances your notebook experience with formatted displays for Session, Driver, and Lap objects, making data exploration intuitive and visually appealing.
...
- **Rich HTML Representations**: Beautiful, formatted displays for all major tif1 objects
...
The Jupyter integration activates automatically when you import `tif1` in a Jupyter environment. The library uses intelligent detection to identify Jupyter notebooks, JupyterLab, and IPython kernels.

### How It Works

When you import `tif1`, the library:
...
- Attempts to import `IPython` module
...
- Returns `True` only if all checks pass, ensuring we're in a genuine Jupyter environment
...
All major tif1 objects provide rich HTML representations that display automatically when you evaluate them in a Jupyter cell. These displays are styled with borders, colors, and structured tables for maximum readability.

### Session Display

The Session object displays comprehensive information about the loaded F1 session, including year, Grand Prix name, session type, backend library, and driver count.
...
- Dynamic driver count (shows "Not loaded" if drivers haven't been fetched yet)
...
The Driver object displays information about a specific driver within a session, including their driver code, session context, and data loading status.
...
While the Jupyter integration activates automatically, you can manually control and customize the display behavior for advanced use cases.

### Manual Activation

If you need to manually enable Jupyter displays (e.g., after importing tif1 before IPython was available), use the `enable_jupyter_display()` function:
...
- Handles errors gracefully if classes can't be imported
...
Check if your code is running in a Jupyter environment:
...
**Note**: The function name starts with underscore (`_is_notebook`) indicating it's an internal API, but it's stable and safe to use.
...
If you prefer standard text output even in Jupyter:
