...
Blobs of 4 KB or larger are zlib-compressed at the SQLite boundary only. The in-memory LRU keeps the plain blob. Small rows stay plain TEXT so legacy cache files remain readable.
