...
title: "CLI Reference"
icon: "terminal"
description: "Command-line interface reference for tif1 - all CLI commands, options, workflows, and best practices"
keywords:
  - "tif1"
  - "Formula 1"
  - "F1 data"
...
The `tif1` CLI provides a command-line interface for exploring Formula 1 data from the terminal. It is built with [Typer](https://typer.tiangolo.com/) for argument parsing. It uses [Rich](https://rich.readthedocs.io/) for colorful terminal output with tables and progress indicators. The CLI requires no configuration.

## What is the tif1 CLI?

The tif1 CLI is a command-line tool. It gives access to Formula 1 data without writing Python code. It is designed for data analysts, F1 enthusiasts, and developers. It serves anyone who wants to explore F1 data from the terminal.

**Key Features:**

- **Zero Configuration**: Works out of the box after installation
- **Rich Terminal Output**: Color-coded tables with Unicode borders
- **Automatic Caching**: SQLite cache speeds up repeated queries
...
2. **Debugging & Development**
   ...
   - Troubleshoot issues with debug logging
...
The CLI is automatically included with a tif1 installation. No additional dependencies, configuration files, or setup steps are required. It works out of the box.
...
After installation, the `tif1` command is available globally in the terminal. The CLI uses the same Python environment as the tif1 installation. This ensures consistency between CLI and programmatic usage.
...
Walk through a complete workflow from discovering events to analyzing lap times:
...
This shows all Grand Prix events for 2025. Note the exact event names - subsequent commands need them.
...
This loads the session data and displays all drivers with their teams. On the first run, the command fetches data from the network (2-10 seconds). Subsequent runs are much faster because data is cached.
...
Check the cache size and location:
...
- **COMMAND**: The action to perform (for example, `events`, `sessions`, `drivers`, `fastest`)
- **ARGUMENTS**: Required positional arguments in a specific order (for example, year, event name, session name)
- **OPTIONS**: Optional flags that modify behavior (for example, `--driver`, `--yes`, `-d`)
...
- Commands use lowercase with hyphens for multi-word names (for example, `cache-info`, `cache-clear`)
- Arguments use natural language names (for example, `year`, `event`, `session`)
...
Understanding these core concepts helps with effective CLI use and helps avoid common pitfalls.
...
Driver codes can change between seasons if a driver switches teams or if there is a code conflict:
...
The CLI implements caching to optimize performance and reduce network requests.
...
2. **Network Fetch**: Data is downloaded from the CDN (StaticDelivr primary; jsDelivr and Hugging Face fallbacks)
...
The CLI uses the Rich library for colorful terminal output with structured formatting.
...
The CLI provides clear, actionable error messages. These messages help with quick identification and resolution of issues.
...
**Cause**: Event name does not match exactly
...
**Cause**: Session name does not match exactly
...
**Cause**: Invalid driver code or driver did not participate
...
**Cause**: Event has not occurred yet or data not published.
...
The data is sourced from tif1's embedded schedule data. It includes historical seasons from 2018 onwards. It also includes current and future seasons as they become available.

**Output Format:**

The command displays a formatted table with:
- **#**: Sequential event number (1-based index)
- **Event**: Official event name (for example, "Monaco Grand Prix", "British Grand Prix")
...
The `sessions` command retrieves all practice, qualifying, sprint, and race sessions for a specific Grand Prix event. Different events may have different session structures (for example, sprint weekends have different formats).
...
The command displays a formatted table with:
- **Driver**: Three-letter driver code (for example, VER, HAM, LEC)
- **Team**: Full team name (for example, Red Bull Racing, Mercedes, Ferrari)
...
**Important Notes:**
- Driver codes are standardized three-letter abbreviations (for example, VER, HAM, LEC)
- Team names reflect the official team name at the time of the event
- Not all drivers may have complete data in all sessions (for example, DNS, DNF)
...
The `cache-info` command provides a summary of the local tif1 cache. The cache stores downloaded F1 data to speed up subsequent requests. This is useful for monitoring cache growth, troubleshooting storage issues, and understanding cache behavior.
...
- **Typical sizes**:
  ...
  - Heavy usage (long analysis sessions): 500+ MB
...
The default cache location can be overridden using the `TIF1_CACHE_DIR` environment variable:
...
The `cache-clear` command removes all cached Formula 1 data from the local cache database. This is useful when:
...
- **Data updates**: Ensuring the latest data comes from the CDN

**Important**: Clearing the cache means all subsequent data requests need a network fetch. These requests are slower until the cache is rebuilt.
...
If the answer is 'n' or Enter is pressed (default is No):
...
- **Cache rebuild**: Automatic as commands are used

**Safety Notes:**

- **No data loss**: Only cached data is removed; original data remains on CDN
- **Reversible**: Cache rebuilds automatically as the CLI is used
- **No configuration changes**: Configuration settings remain intact
...
The `version` command displays the now installed version of tif1. This is useful for:

- **Troubleshooting**: Verify the running version when reporting issues
- **Compatibility**: Check whether the installed version has specific features
- **Updates**: Confirm successful upgrade after running `pip install --upgrade tif1`
- **Documentation**: Ensure documentation matches the installed version
...
- **Troubleshooting**: Diagnose why data is not loading correctly
...
This workflow helps with the following:
...
JSON output is not supported. Parse the table output:
...
2. **Use tab completion** (if the shell supports it):
   ```bash
...
While the CLI primarily uses environment variables, a shell configuration file can hold persistent settings:
...
While CLI does not output JSON directly, table output can be converted:
...
A: No, tif1 CLI does not require any API keys or authentication.
...
A: Now, the CLI focuses on lap times and driver info. Use the Python API for telemetry.
...
A: Ensure tif1 is installed and the Python Scripts directory is in the PATH.

**Q: Permission denied error?**
A: Check cache directory permissions or use a custom cache location.

**Q: Network timeout error?**
A: Increase timeout with `TIF1_TIMEOUT=60` or check the internet connection.
...
Contributions to improve the CLI are welcome. These are the ways to help:

### Reporting Issues

1. Check existing issues on GitHub
2. Provide detailed error messages and steps to reproduce
3. Include the OS, Python version, and tif1 version
