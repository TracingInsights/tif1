...
Load a bundled car image as an RGBA numpy array. The result is cached with `functools.cache`, so repeated calls for the same year and code return the same array without disk reads.
