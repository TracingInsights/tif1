...
icon: "calendar-days"
description: "Schedule data validation, schema management, and internal data structures for Formula 1 event and session information"
keywords:
  - "tif1"
  - "Formula 1"
  - "F1 data"
...
The `schedule_schema` module is the validation layer for the tif1 internal schedule data architecture. It implements a validation system that ensures data integrity and structural consistency across all schedule operations in the library.

## Purpose and Role

The schedule validation system guarantees that all schedule payloads conform to the tif1 internal schema before use by higher-level APIs. This applies to payloads from vendored JSON files, CDN sources, or custom data. This validation layer provides these functions:
...
The validation process is designed for performance and typically completes in under 1 millisecond. It integrates with the tif1 caching system, which minimizes validation overhead in production use.

<Note>
  **For Most Users**: This module operates transparently. Schedule data is accessed through high-level APIs like `get_events()`, `get_sessions()`, and `get_event_schedule()`, which handle validation automatically. Direct use is needed only for advanced scenarios: custom schedule data, debugging, or extending the library.
</Note>

<Warning>
  **Schema Version Compatibility**: Now, only schema version 1 is supported. Attempting to validate payloads with different schema versions will raise an `InvalidDataError`. Future versions of tif1 may introduce new schema versions with additional features or structural changes.
</Warning>
...
3. **Performance Optimization**: Validation is fast (typically &lt;1ms) and runs only once per session due to caching.

4. **Detailed Error Reporting**: When validation fails, the system provides specific error messages that identify what went wrong and where.

5. **Type Safety**: The validation system enforces strict type checking at every level of the data hierarchy. This prevents type-related bugs downstream.
...
The primary and only public validation function in this module. This function performs hierarchical validation of schedule payloads to ensure they conform to tif1's internal schema specification version 1.
...
The function performs validation in a hierarchical, top-down manner. It checks each level of the data structure before it proceeds to the next:
...
Here is a reference of common validation failures and how to fix them:
...
<Tip>
  **Integration Tip**: When building custom schedule loaders or data pipelines, call `validate_schedule_payload()` immediately after payload construction. Call it before any caching or API exposure. This ensures data quality at the earliest stage.
</Tip>
...
The internal schedule schema is event-centric. It is optimized for the most common query patterns in Formula 1 data analysis. This structure differs from the raw f1schedule format, which uses a columnar layout. The event-centric structure gives better performance for event and session lookups.
...
This section provides a reference of all validation rules enforced by `validate_schedule_payload()`.
...
<Info>
  **Early Exit Behavior**: Validation stops at the first rule violation. Only one error is reported at a time, even if multiple issues exist. Fix the reported error and re-validate to discover any additional issues.
</Info>
...
<Tip>
  **Debugging Tip**: When validation fails, print the payload structure with `json.dumps(payload, indent=2)`. Inspect the data hierarchy to identify structural issues.
</Tip>
...
Integrate custom schedule data with the tif1 validation system:
...
Wrap the built-in validation with additional checks:
...
When schema version 2 is introduced, migration logic may be needed:
...
**Symptom**: Error message does not clearly indicate the problem.
...
- **Schema Version 1**: Now the only supported version
...
5. **Use Caching**: Use the tif1 built-in caching to minimize validation overhead
