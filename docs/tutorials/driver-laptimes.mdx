...
The native function exposes the same filters as parameters. `drivers` selects whose laps to plot (a single driver for the view above, or several), `laptime_cutoff` removes slow laps, and `include_deleted` keeps deleted laps when `True`:
