...
icon: "chart-line"
description: "Examine acceleration, position data, and racing lines"
keywords:
  - "telemetry analysis"
  - "speed trace"
  - "throttle analysis"
...
tif1 provides telemetry data including position coordinates and acceleration vectors. This tutorial shows how to use this data for advanced analysis.
...
ver_tel = ver.get_lap(int(ver_fastest["LapNumber"].iloc[0])).telemetry
ham_tel = ham.get_lap(int(ham_fastest["LapNumber"].iloc[0])).telemetry
...
Examine the throttle and brake overlap to analyze driver technique.
...
All of this data is available at high frequency (~4Hz) for every lap. It enables detailed performance analysis.
