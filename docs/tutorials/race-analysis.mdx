...
icon: "flag-checkered"
description: "Analyze a full race with tif1, from data loading to a complete race report"
keywords:
  - "race analysis"
  - "race pace"
  - "lap time analysis"
...
This tutorial shows a complete race analysis workflow. The workflow runs from loading data to generating insights about race pace, strategy, and performance.

<img src="/assets/race_position_changes.png" alt="Race position changes throughout the Abu Dhabi Grand Prix" />
...
print(f"Fastest Lap: {fastest['Driver']} - Lap {fastest['LapNumber']} - {fastest['LapTime'].total_seconds():.3f}s")
```
...
<img src="/assets/race_pace_boxplot.png" alt="Box plot showing race pace distribution for top 5 finishers" />
...
# Box plot of lap times (matplotlib needs numeric values, so convert to seconds)
fig, ax = plt.subplots(figsize=(12, 6))
top_5_laps = clean_laps[clean_laps["Driver"].isin(top_5_drivers)].copy()
...
bp = ax.boxplot(
    [top_5_laps[top_5_laps["Driver"] == d]["LapTimeSeconds"] for d in top_5_drivers],
...
<img src="/assets/race_tire_strategy.png" alt="Tire strategy visualization showing compound usage throughout the race" />
...
def analyze_stint_degradation(laps, driver, stint):
    ...
    # Linear regression (scipy needs numeric values, so convert to seconds)
...
def calculate_gap_to_leader(laps):
    ...
    for lap_num in laps["LapNumber"].unique():
        ...
        # The laps DataFrame stores the session time at lap completion in "Time"
        leader_time = leader["Time"].iloc[0]

        # Calculate gaps (store as seconds for plotting)
...
if len(pit_stops_df) > 0:
    ...
    print(f"Average duration: {pit_stops_df['Duration'].mean().total_seconds():.2f}s")
    print(f"Fastest stop: {pit_stops_df['Duration'].min().total_seconds():.2f}s")
    print(f"Slowest stop: {pit_stops_df['Duration'].max().total_seconds():.2f}s")
...
Compare race pace between drivers on the same compound.
...
def compare_race_pace(laps, drivers, compound="MEDIUM"):
    ...
    for driver in drivers:
        ...
        # Plot lap times vs tire life (matplotlib needs seconds, not timedelta)
...
Create a complete race summary.
...
7. Pit stop analysis
8. Race pace comparison
9. Complete race report

Adapt these patterns to analyze any race in the tif1 database (2018-current).
