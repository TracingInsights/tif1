...
A G-G diagram plots lateral against longitudinal acceleration and shows the performance envelope of a car. The outer boundary represents the maximum grip available. The diagram compares car performance and driver technique clearly.

<img src="/assets/gg_diagram.png" alt="G-G diagram showing acceleration envelope" />
...
fig, ax = tif1.plot_gg_diagram(
    2024, "Monaco", "Q",
    drivers=["VER", "LEC", "NOR"],
    save_path="gg_diagram.png",
    dpi=300,
)
plt.show()
```
...
# One call: loads the session, computes accelerations, and draws the envelope
fig, ax = tif1.plot_gg_diagram(2024, "Monaco", "Q", drivers=["VER", "LEC", "NOR"])
plt.show()

# Or save straight to a file
# tif1.plot_gg_diagram(2024, "Monaco", "Q", save_path="gg_diagram.png")
```

The `smooth_derivative` and `compute_accelerations` helpers above live in the library under `tif1.charts._acceleration`. The [telemetry comparison](/tutorials/telemetry-comparison) chart shares these helpers.
