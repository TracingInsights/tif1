...
The native function exposes the same controls as parameters. `compounds` picks which compounds to plot, `fuel_correction` sets the per-lap fuel gain, `min_laps` drops compounds with too few laps, and `smoothing_window` tunes the rolling median:
...
The whole workflow above (fuel correction, rolling median, cliff-friendly y-limits) is wrapped in a single native function:
