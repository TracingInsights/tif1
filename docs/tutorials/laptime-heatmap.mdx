...
The native function exposes the same filters as parameters. `laptime_cutoff` fixes the color scale (fastest to `fastest * cutoff`), `include_deleted` and `include_pit_laps` keep those laps when `True`, and `cmap` / `xticklabels` control the styling:

```python
import tif1

# Defaults match the chart above
fig, ax = tif1.plot_laptime_heatmap(2023, 'Monaco Grand Prix', 'R')
plt.show()

# Custom color scale and tick spacing
fig, ax = tif1.plot_laptime_heatmap(
...
# One call: loads the race, pivots the data, and draws the heatmap
fig, ax = tif1.plot_laptime_heatmap(2023, 'Monaco Grand Prix', 'R')
plt.show()

# Or save straight to a file
# tif1.plot_laptime_heatmap(2023, 'Monaco Grand Prix', 'R', save_path="laptime_heatmap.png")
```
...
```python
# Select the 5 fastest drivers by average lap time
top_drivers = driver_avg.index[:5]
