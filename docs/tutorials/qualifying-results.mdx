...
The native function exposes the shared filters. `n_drivers` keeps only the top-N finishers, and `include_deleted` controls whether deleted laps count toward a driver's best (the default `False` is a correctness fix: a deleted lap can never win the pole):
