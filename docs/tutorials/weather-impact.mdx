...
# Matplotlib and numpy need numeric values, so convert the timedelta lap times to seconds
clean_laps["LapTimeSeconds"] = clean_laps["LapTime"].dt.total_seconds()

# Plot track temp vs lap time
plt.figure(figsize=(12, 6))
plt.scatter(clean_laps["TrackTemp"], clean_laps["LapTimeSeconds"], alpha=0.3)
plt.xlabel("Track Temperature (°C)")
plt.ylabel("Lap Time (s)")
plt.title("Track Temperature Impact on Lap Times")

# Add trend line
z = np.polyfit(clean_laps["TrackTemp"], clean_laps["LapTimeSeconds"], 1)
p = np.poly1d(z)
...
if weather["Rainfall"].any():
    ...
    print(f"Rain started at: {rain_start.total_seconds() / 60:.1f} minutes")

    # Mark laps as wet or dry
    # The laps DataFrame stores the session time at lap completion in "Time"
    laps["Wet"] = laps["Time"] >= rain_start

    # Compare dry vs wet lap times
    dry_laps = laps[laps["Wet"] == False]
    wet_laps = laps[laps["Wet"] == True]

    print(f"Dry avg: {dry_laps['LapTime'].mean().total_seconds():.3f}s")
    print(f"Wet avg: {wet_laps['LapTime'].mean().total_seconds():.3f}s")
    print(f"Difference: {(wet_laps['LapTime'].mean() - dry_laps['LapTime'].mean()).total_seconds():.3f}s")

    # Visualize (matplotlib needs seconds, not timedelta)
    plt.figure(figsize=(12, 6))
    plt.scatter(dry_laps["LapNumber"], dry_laps["LapTime"].dt.total_seconds(),
                label="Dry", alpha=0.5, color="orange")
    plt.scatter(wet_laps["LapNumber"], wet_laps["LapTime"].dt.total_seconds(),
                label="Wet", alpha=0.5, color="blue")
    # Mark the first lap completed after the rain started
    rain_lap = laps[laps["Wet"]]["LapNumber"].min()
    plt.axvline(x=rain_lap, color="red", linestyle="--",
                label="Rain Start")
...
# Convert the timedelta lap times to seconds for numeric analysis
clean_laps["LapTimeSeconds"] = clean_laps["LapTime"].dt.total_seconds()
...
compound_temp = (
    pd.DataFrame(clean_laps)
    .groupby(["Compound", "TempRange"])["LapTimeSeconds"]
    .mean()
    .reset_index()
)
...
for compound in ["SOFT", "MEDIUM", "HARD"]:
    ...
    plt.plot(data["TempRange"], data["LapTimeSeconds"],
...
# scikit-learn needs numeric values, so use the lap time in seconds
...
clean_laps["WeatherAdjusted"] = clean_laps["LapTimeSeconds"] - clean_laps["ExpectedLapTime"]
