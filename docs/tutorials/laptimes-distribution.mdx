...
# race.drivers returns driver numbers, so translate them to abbreviations first
point_finishers = [race.get_driver(i)["Abbreviation"] for i in race.drivers[:10]]
print(f"Point finishers: {point_finishers}")

# Pick quick laps only (filter out slow laps)
driver_laps = laps.pick_drivers(point_finishers).copy()
...
# Use transform to keep every column; groupby().apply() drops the grouping column
fastest_per_driver = driver_laps.groupby("Driver")["LapTime"].transform("min")
driver_laps = driver_laps[driver_laps["LapTime"] < fastest_per_driver * 1.10]
...
```python
# point_finishers already holds the abbreviations in finishing order
finishing_order = point_finishers
...
The native function exposes the same filters as parameters. `n_drivers` selects how many finishers to include (10 by default, matching the point finishers above), and `laptime_cutoff` controls the per-driver outlier removal:

```python
import tif1

# Top 10 point finishers (default)
fig, ax = tif1.plot_laptimes_distribution(2023, "Azerbaijan", 'R')
plt.show()

# More drivers, relaxed outlier cutoff
fig, ax = tif1.plot_laptimes_distribution(
    2023, "Azerbaijan", 'R',
    n_drivers=15,
    laptime_cutoff=1.15,
)
...
Drivers are always ordered by finishing position, and deleted/pit laps are always excluded.
...
# One call: loads the race, filters the laps, and draws violin + swarm plots
fig, ax = tif1.plot_laptimes_distribution(2023, "Azerbaijan", 'R')
plt.show()

# Or save straight to a file
# tif1.plot_laptimes_distribution(2023, "Azerbaijan", 'R', save_path="laptimes_distribution.png")
```
...
# Seaborn needs numeric values, so convert the timedelta lap times to seconds
quali_laps["LapTime(s)"] = quali_laps["LapTime"].dt.total_seconds()
